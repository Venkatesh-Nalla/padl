...
public class Utils {
	...
	static char[][] getQualifiedName(
		...
		//TODO Petrillo - Review!!! ICPPASTName to  ICPPASTNameSpecifier
		//final ICPPASTName[] names = aQualifiedName.getNames();
		final ICPPASTNameSpecifier[] names = aQualifiedName.getAllSegments();
		final char[][] qualifiedNameArray = new char[names.length][];
		for (int i = 0; i < names.length; i++) {
			final ICPPASTNameSpecifier name = names[i];
	 ...
	 * C++ il suffit qu'une m�thode soit d�clar� virtuelle pur pour que la
	...
	static Boolean isAbstract(final ICPPClassType aCPPClass) {
		// On parcourt toutes les m�thodes d�clar�es par la classe
		for (final ICPPMethod method : aCPPClass.getDeclaredMethods()) {
			// Si une m�thode est virtuelle pure on renvoie true
			if (method.isPureVirtual()) {
				return true;
			}
		}

		// Si on a rien trouv� c'est que la classe n'est pas abstraite !!
		return false;
	}
